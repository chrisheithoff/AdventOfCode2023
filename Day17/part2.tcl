...
proc get_next_states {state} {
    ...
    set possible_states [list]
    lassign $state loc dir streak


    # Right/left turns allowed for streak at least 4. (reset streak to 1)
    if {$streak >= 4} {
        # Right turn 
        set new_dir [dict get "E S S W W N N E" $dir]
        set new_loc [get_new_loc $loc $new_dir]
        lappend possible_states [list $new_loc $new_dir 1]
    ...
    # Straight allowed for streak less than 10. (increment streak by 1)
    if {$streak < 10} {
        set new_dir $dir
        set new_loc [get_new_loc $loc $dir]
        lappend possible_states [list $new_loc $new_dir [incr streak]]
    }

    # Validate states (cannot be offgrid or at end point unable to stop)
    foreach possible_state $possible_states {
        lassign $possible_state loc dir streak
        if {![is_valid_loc $loc]} {
            continue
        }

        if {$loc == $end_loc && $streak < 4} {
            continue
        }
        lappend next_states $possible_state
    }
    return $next_states
}

proc print_path {state} {
    global grid
    global from
    global visited
...
while {1} {
    ...
    set came_from        [dict get $from      $state]
    puts "Visited state = $state ($heat_loss_so_far)"

    # Step 1:  Mark state as visited
    dict incr  visited   $state

    # Are we there yet?  Can have we traveled enough to be able to stop?
    lassign $state loc dir streak
    if {$loc == $end_loc && $streak >=4 } {
        puts "We're here!   {$end_loc} streak = $streak"
        break
    }
    ...
    foreach next_state $next_states {
        ...
        # puts "    next = $next_state"
        # lappend unvisited  [list $next_state $priority]
        set insertion_spot [expr {[lsearch -integer -index 1 -bisect $unvisited $priority]+1}]
...
set part2_answer [dict get $heat_loss $state]
